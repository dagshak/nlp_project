...
def train(args):
            ...
            for m in input_encoder.modules():
                if isinstance(m, nn.Linear):
                    ...
                    if m.bias is not None:
                        '''
                        removed if statement. not valid for non-cuda tensors
                        '''
    ...
    for i in range(len(test_batches)):
        ...
        test_src_batch = Variable(test_src_batch)
        test_tgt_batch = Variable(test_tgt_batch)
        test_lbl_batch = Variable(test_lbl_batch)
